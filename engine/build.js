...
const ORDER = [
  ...
  'src/agents/judge/prompts.ts', 'src/agents/judge/index.ts',
...
function assertBundled(spec, fileDir, mod) {
  if (!spec.startsWith('.')) {
    fail(mod + ': imports external package "' + spec + '" — the Workflow sandbox has no module loader.\n' +
  ...
  if (!resolvable.has(target)) {
    fail(mod + ': imports "' + spec + '" but its target is not bundled — its symbols would be undefined in the flat scope.\n' +
