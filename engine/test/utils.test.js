...
import { plain, norm, lab, padIdx, lastScore, openLine, resultSoFarMd, waveMd, render } from '../src/utils/index.js'
...
describe('plain', () => {
  ...
  it('renders an array one "- el" per line', () => expect(plain(['a', 'b', 'c'])).toBe('- a\n- b\n- c'))
  it('renders an object as key: value lines', () => expect(plain({ a: 1, b: 'two' })).toBe('a: 1\nb: two'))
  ...
  it('indents nested array elements two spaces after the dash', () => {
    expect(plain([{ k: 'one', v: 1 }, { k: 'two', v: 2 }])).toBe('- k: one\n  v: 1\n- k: two\n  v: 2')
  })
...
describe('norm', () => {
  ...
  it('handles null/empty', () => { expect(norm('')).toBe(''); expect(norm(null)).toBe('') })
})
...
describe('lastScore', () => {
  it('returns the last scoreHistory score, else null', () => {
    expect(lastScore({ scoreHistory: [{ wave: 0, score: 60 }, { wave: 1, score: 80 }] })).toBe(80)
...
describe('openLine', () => {
  it('renders a scored entry', () => {
    expect(openLine({ id: 7, keyword: 'hnsw', why: 'knobs', scoreHistory: [{ wave: 1, score: 80 }] })).toBe('#7 [80] hnsw — knobs')
  })
  it('renders an unscored entry as "new"', () => {
    expect(openLine({ id: 8, keyword: 'sharding', why: 'scale', scoreHistory: [] })).toBe('#8 [new] sharding — scale')
  })
})
...
describe('resultSoFarMd', () => {
  ...
  it('omits the working block when empty and shows _none_ for empty lists', () => {
    const md = resultSoFarMd({ answer: 'A', confidence: 'low', working: '', evidence: [], resolved: [], openGaps: [], tensions: [] })
...
describe('waveMd', () => {
  it('renders a wave block with picks + open store', () => {
    const coord = { stop: { done: false, reason: 'more to verify' }, resultSoFar: { answer: 'A', confidence: 'med', working: '', evidence: [], resolved: [], openGaps: [], tensions: [] } }
    const picks = [{ id: 1, keyword: 'hnsw', why: 'knobs', score: 80, path: [], sources: ['arXiv'] }]
    const store = [{ id: 1, keyword: 'hnsw', scoreHistory: [{ wave: 1, score: 80 }] }, { id: 2, keyword: 'sharding', scoreHistory: [] }]
...
describe('render', () => {
  ...
  it('renders an absent key as empty string', () => expect(render('[{{missing}}]\n', {})).toBe('[]'))
